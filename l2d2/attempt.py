...
def run_eval(p, eval_data: Tuple[jnp.array, jnp.array], carry_len: int) -> float:
    ...
    probs = zoo.step(preprocess.samples_to_input(samples, zoo.VALUE_OPTS), p, carry_len)
...
def _do_train(opt_state, eval_data, epochs: int, batch_size: int, carry_len: int,
    ...
    for epoch in range(epochs):
        ...
        while True:
            ...
            assert len(item.lengths) == batch_size
            assert len(item.bytes) == batch_size
            samples = preprocess.samples_to_input(item.bytes, zoo.VALUE_OPTS)
            opt_state = zoo.train_step(stepno, opt_state, samples,
                                       jnp.array(item.lengths, dtype='uint8'), carry_len, get_params, opt_update)
            stepno += 1
            sys.stdout.write('.')
            if stepno % 64 == 0:
                print(' ' + compute_rate())
...
def run_train(opts) -> None:
    ...
    with scoped_time('getting eval data'):
        eval_data = sampler.get_eval_data(data, opts.batch_size, opts.eval_minibatches)
    ...
    try:
        _do_train(opt_state, eval_data, opts.epochs, opts.batch_size, opts.carry_len, opts.steps_per_eval, q, stat_recorder, get_params, opt_update)
